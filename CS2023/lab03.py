...
def skip2_add(n):
    ...
    22
    """
    if n <= 0:  # conditional to avoid errors
        return 0 
    else:
...
def a(n):
    ...
    if n == 1: # a(1) = 1
        ...
        return int ((3/2) * a(n-1)) # code/calculation for if a(n-1) is even
    else:
        return int ((3/2) * (a(n-1) + 1)) # code for if a(n-1) is not even
...
def paths(m, n):
    ...
    if m == 1 or n == 1: # given that a 1 by n path has only one known path
        return 1
    else:
